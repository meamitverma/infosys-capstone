...
lowestCompletion = aggDF.agg(min('AverageCompletionPercent').alias('LowestCompletion')).first()[0]
showDF = aggDF.filter(col('AverageCompletionPercent') == lowestCompletion)
showDF.show()
